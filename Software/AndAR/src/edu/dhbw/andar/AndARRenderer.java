...
public class AndARRenderer implements Renderer, PreviewFrameSink {
	 ...
	 *            non AR renderer, may be null
	 */
	public AndARRenderer(Resources res, ARToolkit markerInfo/*
															 * , AndARActivity
															 * activity
															 */) {
		this.res = res;
	...
	public final void onDrawFrame(GL10 gl) {
		...
		if (takeScreenshot) {
			...
			gl.glReadPixels(0, 0, screenWidth, screenHeight, GL10.GL_RGBA,
					GL10.GL_UNSIGNED_BYTE, screenshotBuffer);
			for (int i = 0; i < screenHeight; i++) {// remember, that OpenGL
													// bitmap is incompatible
													// with Android bitmap
													// and so, some correction
													// need.
			...
			this.screenshot = Bitmap.createBitmap(screenshot, screenWidth,
					screenHeight, Config.RGB_565);
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		square = new float[] { -100f * aspectRatio, -100.0f, -1f,
				100f * aspectRatio, -100.0f, -1f, -100f * aspectRatio, 100.0f,
				-1f, 100f * aspectRatio, 100.0f, -1f };
	 ...
	 * 
	 * @see
	 * edu.dhbw.andopenglcam.interfaces.PreviewFrameSink#setNextFrame(java.nio
	 * .ByteBuffer)
	 */
	@Override
	 ...
	 */
	@Override
	public void setPreviewFrameSize(int textureSize, int realWidth,
			int realHeight) {
		...
		this.textureCoords = new float[] {
				...
				((float) realWidth) / textureSize,
				((float) realHeight) / textureSize, 0.0f, 0.0f,
