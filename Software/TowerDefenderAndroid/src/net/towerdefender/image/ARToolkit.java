...
public class ARToolkit {
	...
	/**
	 * Every object get'S his own unique ID. This counter may never be
	 * decremented.
	 */
	private int nextObjectID = 0;
	/**
	 * The transformation matrix is accessed, when drawing the object(read), but
	 * is also written to when detecting the markers from a different thread.
	 */
	...
	/**
	 * absolute path of the local files: the calib file will be stored there,
	 * among other things
	 */
	...
	/**
	 * Registers an object to the ARToolkit. This means: The toolkit will try to
	 * determine the pose of the object. If it is visible the draw method of the
	 * object will be invoked. The corresponding translation matrix will be
	 * applied inside opengl before doing so. TODO: registering a object with
	 * the same pattern twice will not work, as arloadpatt will create different
	 * IDs for the same pattern, and the detecting function will return only the
	 * first id as being detected. we need to store patt load id's in an hash ->
	 * loadpatt as a native function returning the ID -> pass this id to the
	 * object registering function.
	 * 
	 * @param arobject
	...
	/* throws AndARException */{
		...
		try {
			...
			IO.transferFileToPrivateFS(baseFolder, arobject.getPatternName(),
					res);
	...
	/**
	 * Register a object to the native library. From now on the detection
	 * function will determine if the given object is visible on a marker, and
	 ...
	 */
	private native void addObject(int id, ARObject obj, String patternName,
	 ...
	 */
	private native void artoolkit_init(String filesFolder, int imageWidth,
	 ...
	 *            the image
	 * @param matrix
	 *            the transformation matrix for each marker, will be locked
	 *            right before the trans matrix will be altered
	 * @return number of markers
	 */
	private native int artoolkit_detectmarkers(byte[] in, Object transMatMonitor);

	/**
	 * Inverse a three by four matrix. Matrix has to be 3 by 4! (but is actually
	 * a 4 by 4 homogene matrix.)
	 * 
	 * @param mat1
	...
	/**
	 * Multiply one (three by four) matrix by another. Matrix has to be 3 by 4!
	 * (but is actually a 4 by 4 homogene matrix.)
	 * 
	 * @param multiplier
	 * @param multiplicand
	 * @param result
	 ...
	 * @return
	 */
	public native static int arUtilMatMul(double[] multiplier,
			double[] multiplicand, double[] result);
	...
	public void setScreenSize(int width, int height) {
		// if (Config.DEBUG)
		Log.i("MarkerInfo", "setting screen width(" + width + ") and height("
				+ height + ")");
	...
	protected void setImageSize(int width, int height) {
		// if ()
		Log.i("MarkerInfo", "setting image width(" + width + ") and height("
				+ height + ")");
	...
	private void initialize() {
		...
		if (screenWidth > 0 && screenHeight > 0 && imageWidth > 0
				&& imageHeight > 0) {
			// if (Config.DEBUG)
			Log.i("MarkerInfo", "going to initialize the native library now");
			artoolkit_init(baseFolder + File.separator + calibFileName,
					imageWidth, imageHeight, screenWidth, screenHeight);
			ARObject.glCameraMatrixBuffer = GraphicsUtil
					.makeFloatBuffer(ARObject.glCameraMatrix);
			// if (Config.DEBUG)
	...
	/**
	 * Detects the markers in the image, and updates the state of the MarkerInfo
	 * accordingly.
	 * 
	 * @param image
	 */
