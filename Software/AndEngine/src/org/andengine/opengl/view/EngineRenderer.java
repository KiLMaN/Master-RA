...
public class EngineRenderer implements GLSurfaceView.Renderer {
	...
	public EngineRenderer(final Engine pEngine,
			final ConfigChooser pConfigChooser,
			final IRendererListener pRendererListener) {
		this.mEngine = pEngine;
		this.mConfigChooser = pConfigChooser;
		this.mRendererListener = pRendererListener;
		this.mGLState = new GLState();
		this.mMultiSampling = this.mEngine.getEngineOptions()
	...
	public void onSurfaceCreated(final GL10 pGL, final EGLConfig pEGLConfig) {
		synchronized (GLState.class) {
			final RenderOptions renderOptions = this.mEngine.getEngineOptions()
					.getRenderOptions();
			this.mGLState.reset(renderOptions, this.mConfigChooser, pEGLConfig);

			// TODO Check if available and make available through
			...
			/*
			 * Enabling culling doesn't really make sense, because triangles are
			 * never drawn 'backwards' on purpose.
			 */
	...
	public void onSurfaceChanged(final GL10 pGL, final int pWidth,
		...
		if (this.mRendererListener != null) {
			this.mRendererListener.onSurfaceChanged(this.mGLState, pWidth,
					pHeight);
		}
	}
	...
	public void onDrawFrame(final GL10 pGL) {
		...
		synchronized (GLState.class) {
			if (this.mMultiSampling
					&& this.mConfigChooser.isCoverageMultiSampling()) {
