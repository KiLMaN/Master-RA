...
public class DebugTimer {
	...
	public void dump(final boolean pClear) {
		final long now = System.currentTimeMillis();
		if(this.mDebugTimes.size() > 1) {
			Debuging.w(this.getClass().getSimpleName() + " not all ended!");
		}
	...
	public class DebugTime {
		...
		public void dump(final int pIndent, final String pPostfix) {
			if(this.mSplit) {
				...
				Arrays.fill(indent, ' ');
				Debuging.log(DebugTimer.this.mDebugLevel, new String(indent) + SPLIT_STRING + "'" + this.mLabel + "'" + " @( " + (this.mEndTime - this.mStartTime) + "ms )" + pPostfix);
			} else {
				final char[] indent = new char[pIndent * INDENT_SPACES];
				Arrays.fill(indent, ' ');
				if(this.mChildren == null) {
					Debuging.log(DebugTimer.this.mDebugLevel, new String(indent) + "'" + this.mLabel + "' @( " + (this.mEndTime - this.mStartTime) + "ms )" + pPostfix);
				} else {
					final ArrayList<DebugTime> children = this.mChildren;
					final int childCount = children.size();
					
					Debuging.log(DebugTimer.this.mDebugLevel, new String(indent) + "'" + this.mLabel + "' {");
					for(int i = 0; i < childCount - 1; i++) {
						children.get(i).dump(pIndent + 1, ",");
					}
					children.get(childCount - 1).dump(pIndent + 1);
					Debuging.log(DebugTimer.this.mDebugLevel, new String(indent) + "}@( " + (this.mEndTime - this.mStartTime) + "ms )" + pPostfix);
				}
			}
		}
