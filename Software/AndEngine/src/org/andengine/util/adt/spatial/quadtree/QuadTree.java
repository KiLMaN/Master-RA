...
public abstract class QuadTree<B extends IBounds, T extends ISpatialItem<B>>
	...
	public String toString() {
		final StringBuilder sb = new StringBuilder().append('[')
				...
				.append('\n').append('\t').append("MaxLevel: ")
				.append(this.mMaxLevel).append(',').append('\n').append('\t')
				.append("Bounds: ");
	...
	public synchronized void move(final T pItem, final B pBounds)
		...
		if (!success) {
			throw new AndEngineRuntimeException("Failed to remove item: '"
					+ pItem.toString() + " from old bounds: '"
					+ pBounds.toString() + "'.");
		}
		this.add(pItem);
	}
	 ...
	 */
	@Deprecated
	public synchronized void move(final T pItem, final B pOldBounds,
			final B pNewBounds) throws AndEngineRuntimeException {
	...
	public synchronized <L extends List<T>> L query(final B pBounds,
			final L pResult) {
	 ...
	 */
	public synchronized <L extends List<S>, S extends T> L queryForSubclass(
			final B pBounds, final IMatcher<T> pMatcher, final L pResult)
			throws ClassCastException {
	...
	public synchronized void callItems(
			final ParameterCallable<T> pParameterCallable) {
		this.mRoot.callItems(pParameterCallable);
	}

	public synchronized void callNodes(
			final ParameterCallable<QuadTreeNode> pParameterCallable) {
	...
	public abstract class QuadTreeNode implements IBounds {
		...
		public boolean hasChildren() {
			return this.mTopLeftChild == null && this.mTopRightChild == null
		...
		protected abstract boolean contains(final BoundsSplit pBoundsSplit,
				final B pBounds);
		...
		protected abstract void appendBoundsToString(
		...
		public String toString(final int pIndent) {
			...
			final StringBuilder sb = new StringBuilder().append(indents)
					.append('[').append(" Class: ")
			...
			/* Children */
			if (this.mTopLeftChild == null && this.mTopRightChild == null
					&& this.mBottomLeftChild == null
					&& this.mBottomRightChild == null) {
				sb.append(']');
			} else {
		...
		public <L extends List<T>> L getItemsAndItemsBelow(
				final IMatcher<T> pMatcher, final L pResult) {
			if (this.mItems != null) {
		...
		public <L extends List<S>, S extends T> L getItemsAndItemsBelowForSubclass(
				final IMatcher<T> pMatcher, final L pResult)
				throws ClassCastException {
			if (this.mItems != null) {
				final int itemCount = this.mItems.size();
			...
			if (this.mTopLeftChild != null) {
				this.mTopLeftChild.getItemsAndItemsBelowForSubclass(pMatcher,
						pResult);
			}
			if (this.mTopRightChild != null) {
				this.mTopRightChild.getItemsAndItemsBelowForSubclass(pMatcher,
						pResult);
			}
			if (this.mBottomLeftChild != null) {
				this.mBottomLeftChild.getItemsAndItemsBelowForSubclass(
						pMatcher, pResult);
			}
			if (this.mBottomRightChild != null) {
				this.mBottomRightChild.getItemsAndItemsBelowForSubclass(
						pMatcher, pResult);
			}
		...
		public <L extends List<T>> L query(final B pBounds, final L pResult) {
			...
			if (this.queryChild(pBounds, pResult, this.mTopLeftChild)) {
				...
				return pResult;
					...
					.queryChild(pBounds, pResult, this.mBottomRightChild)) {
				return pResult;
			} else {
				return pResult;
			}
		...
		public <L extends List<T>> L query(final B pBounds,
				final IMatcher<T> pMatcher, final L pResult) {
			/* Test against all items in this node. */
			if (this.mItems != null) {
				for (final T item : this.mItems) {
					if (this.intersects(pBounds, item.getBounds())
							&& pMatcher.matches(item)) {
						pResult.add(item);
					}
				}
			}
			...
			if (this.queryChild(pBounds, pMatcher, pResult, this.mTopLeftChild)) {
				...
				return pResult;
					...
					this.mBottomLeftChild)) {
				return pResult;
			} else if (this.queryChild(pBounds, pMatcher, pResult,
					this.mBottomRightChild)) {
				return pResult;
			} else {
				return pResult;
			}
		...
		@SuppressWarnings("unchecked")
		public <L extends List<S>, S extends T> L queryForSubclass(
				final B pBounds, final IMatcher<T> pMatcher, final L pResult)
				throws ClassCastException {
			/* Test against all items in this node. */
			if (this.mItems != null) {
		 ...
		 */
		private <L extends List<T>> boolean queryChild(final B pBounds,
				final L pResult, final QuadTreeNode pChild) {
			if (pChild == null) {
				return false;
			}
		 ...
		 *         <code>false</code> otherwise.
		 */
		private <L extends List<S>, S extends T> boolean queryChildForSubclass(
				final B pBounds, final IMatcher<T> pMatcher, final L pResult,
				final QuadTreeNode pChild) throws ClassCastException {
			if (pChild == null) {
				return false;
			}
		...
		public boolean containsAny(final B pBounds, final IMatcher<T> pMatcher) {
			...
			if (this.containsAnyChild(pBounds, pMatcher, this.mTopLeftChild)) {
				return true;
			} else if (this.containsAnyChild(pBounds, pMatcher,
					this.mTopRightChild)) {
				return true;
			} else if (this.containsAnyChild(pBounds, pMatcher,
					this.mBottomLeftChild)) {
		...
		private boolean containsAnyChild(final B pBounds,
				final IMatcher<T> pMatcher, final QuadTreeNode pChild) {
			if (pChild == null) {
				return false;
			}

			if (pChild.intersects(pBounds)
					&& pChild.containsAny(pBounds, pMatcher)) {
				return true;
			}
		...
		public void add(final T pItem, final B pBounds)
			...
			if (this.mTopLeftChild != null
					&& this.mTopLeftChild.contains(pBounds)) {
			...
			if (this.mTopRightChild != null
					&& this.mTopRightChild.contains(pBounds)) {
			...
			if (this.mBottomLeftChild != null
					&& this.mBottomLeftChild.contains(pBounds)) {
				this.mBottomLeftChild.add(pItem, pBounds);
				return;
			} else if (this.contains(BoundsSplit.BOTTOM_LEFT, pBounds)) {
				if (this.mBottomLeftChild == null) {
					try {
						this.mBottomLeftChild = this
								.split(BoundsSplit.BOTTOM_LEFT);
						this.mBottomLeftChild.add(pItem, pBounds);
					} catch (final BoundsSplitException e) {
						this.addItemSafe(pItem);
					}
			...
			if (this.mBottomRightChild != null
					&& this.mBottomRightChild.contains(pBounds)) {
				this.mBottomRightChild.add(pItem, pBounds);
				return;
			} else if (this.contains(BoundsSplit.BOTTOM_RIGHT, pBounds)) {
				if (this.mBottomRightChild == null) {
					try {
						this.mBottomRightChild = this
								.split(BoundsSplit.BOTTOM_RIGHT);
						this.mBottomRightChild.add(pItem, pBounds);
					} catch (final BoundsSplitException e) {
						this.addItemSafe(pItem);
					}
		...
		public boolean remove(final T pItem, final B pBounds)
				throws IllegalArgumentException {
			if (!this.contains(pBounds)) { // TODO Perform this check only for
											// the root?
				throw new IllegalArgumentException("pItem (" + pItem.toString()
						+ ") is out of the bounds of this "
						+ this.getClass().getSimpleName() + ".");
			}
			...
			} else if (this.mTopRightChild != null
					&& this.mTopRightChild.contains(pBounds)) {
				return this.mTopRightChild.remove(pItem, pBounds);
			} else if (this.mBottomLeftChild != null
					&& this.mBottomLeftChild.contains(pBounds)) {
				return this.mBottomLeftChild.remove(pItem, pBounds);
			} else if (this.mBottomRightChild != null
					&& this.mBottomRightChild.contains(pBounds)) {
