...
public class GameConfig {
	...
	static {
		...
		/*
		 * defaultTower.add(tower); tower = new Tower(new Position(30, 30),
		 * defaultArsenal); defaultTower.add(tower); tower = new Tower(new
		 * Position(60, 60), defaultArsenal); defaultTower.add(tower); tower =
		 * new Tower(new Position(70, 70), defaultArsenal);
	...
	static {
		...
		parserTower.loadFile();
		Node root = parserTower.getRoot();
		NodeList list = parserTower.getSubNodes(root, "tower");
		if (list != null) {
			for (int i = 0; i < list.getLength(); i++) {
				Node positionsNode = parserTower.getSubNode(list.item(i),
						"position");
		...
		parserWaves.loadFile();
		Node rootWave = parserWaves.getRoot();
		NodeList listWave = parserWaves.getSubNodes(rootWave, "wave");
		if (list != null) {
			for (int i = 0; i < listWave.getLength(); i++) {
				NodeList enemiesRoot = parserWaves.getSubNodes(
						listWave.item(i), "enemie");
